...
Session injection:
  The vault manages its own sessions via session_factory.
  Tests inject the test pg_session_factory so the vault uses the test DB.
"""
...
class TestFernetVaultCRUD:
    ...
    async def test_store_and_get_round_trip(
        ...
        # Flush the tenant insert first so FK constraint is satisfied
        await db_session.flush()

        await vault.store(tid, "api_key", "super-secret-value")
        result = await vault.get(tid, "api_key")

        assert result == "super-secret-value"

    @pytest.mark.asyncio
    async def test_store_encrypts_at_rest(
        self, vault: FernetPostgresVault, db_session, tenant_a
    ):
        ...
        await vault.store(tid, "secret", "plaintext-value")
    ...
    async def test_delete_removes_row(
        ...
        await vault.store(tid, "temp_key", "temp-value")
        await vault.delete(tid, "temp_key")

        with pytest.raises(SecretNotFound):
            await vault.get(tid, "temp_key")
    ...
    async def test_get_missing_key_raises_secret_not_found(
        ...
        with pytest.raises(SecretNotFound):
            await vault.get(tid, "nonexistent_key")
    ...
    async def test_delete_missing_key_raises_secret_not_found(
        ...
        with pytest.raises(SecretNotFound):
            await vault.delete(tid, "nonexistent_key")
    ...
    async def test_list_keys_returns_names_only(
        ...
        await vault.store(tid, "key_alpha", "value_alpha")
        await vault.store(tid, "key_beta", "value_beta")

        keys = await vault.list_keys(tid)
    ...
    async def test_upsert_overwrites_existing_secret(
        ...
        await vault.store(tid, "token", "old-value")
        await vault.store(tid, "token", "new-value")

        result = await vault.get(tid, "token")
    ...
    @pytest.mark.asyncio
    async def test_bytea_round_trip(
        self, vault: FernetPostgresVault, db_session, tenant_a
    ):
        ...
        await vault.store(tid, "unicode_key", "héllo wörld — ñ")
        result = await vault.get(tid, "unicode_key")
...
class TestCrossTenantIsolation:
    ...
    async def test_tenant_a_cannot_read_tenant_b_secret(
        ...
        await vault.store(tid_b, "api_key", "secret_B")

        with pytest.raises(SecretNotFound):
            # A requests B's key — must NOT return B's secret
            await vault.get(tid_a, "api_key")
    ...
    async def test_same_key_name_per_tenant_is_independent(
        ...
        await vault.store(tid_a, "api_key", "secret_A")
        await vault.store(tid_b, "api_key", "secret_B")

        assert await vault.get(tid_a, "api_key") == "secret_A"
        assert await vault.get(tid_b, "api_key") == "secret_B"
    ...
    async def test_list_keys_is_tenant_scoped(
        ...
        await vault.store(tid_a, "key_only_for_a", "value_a")
        await vault.store(tid_b, "key_only_for_b", "value_b")

        keys_a = await vault.list_keys(tid_a)
        keys_b = await vault.list_keys(tid_b)
...
class TestAuditLog:
    ...
    async def test_store_writes_audit_row(
        ...
        await vault.store(tid, "key1", "val1")
    ...
    async def test_get_writes_audit_row(
        ...
        await vault.store(tid, "key2", "val2")
        await vault.get(tid, "key2")
    ...
    async def test_delete_writes_audit_row(
        ...
        await vault.store(tid, "key3", "val3")
        await vault.delete(tid, "key3")
    ...
    async def test_failed_get_still_writes_audit_row(
        ...
        with pytest.raises(SecretNotFound):
            await vault.get(tid, "missing_key")
    ...
    async def test_store_twice_produces_two_audit_rows(
        ...
        await vault.store(tid, "key", "value-1")
        await vault.store(tid, "key", "value-2")
    ...
    async def test_audit_log_has_no_plaintext_or_ciphertext(
        ...
        await vault.store(tid, "secret_key", "my-plaintext-value")
