...
async def test_build_tts_openai_returns_livekit_plugin(config_openai_tts):
    with patch("core.tts.openai_tts.OpenAITTS") as mock_openai_tts:
        ...
        result = await build_tts(config_openai_tts)

        assert result is mock_plugin


async def test_build_tts_openai_passes_voice_id(config_openai_tts):
    with patch("core.tts.openai_tts.OpenAITTS") as mock_openai_tts:
        mock_openai_tts.return_value = MagicMock()

        await build_tts(config_openai_tts)

        call_kwargs = mock_openai_tts.call_args[1]
        assert call_kwargs["voice"] == "ash"


async def test_build_tts_openai_passes_instructions_from_config(config_openai_tts):
...
async def test_build_tts_openai_passes_speed_from_voice_settings(config_openai_tts):
...
async def test_build_stt_openai_returns_livekit_plugin(config_openai_stt):
    with patch("core.stt.openai_stt.OpenAISTT") as mock_openai_stt:
        ...
        result = await build_stt(config_openai_stt)

        assert result is mock_plugin


async def test_build_stt_openai_passes_model_language_use_realtime(config_openai_stt):
    config_openai_stt["stt_use_realtime"] = False
    with patch("core.stt.openai_stt.OpenAISTT") as mock_openai_stt:
        mock_openai_stt.return_value = MagicMock()

        await build_stt(config_openai_stt)
...
async def test_build_tts_unknown_provider_raises_value_error():
    with pytest.raises(ValueError, match="nonexistent"):
        await build_tts({"tts_provider": "nonexistent"})


async def test_build_stt_unknown_provider_raises_value_error():
    with pytest.raises(ValueError, match="nonexistent"):
        await build_stt({"stt_provider": "nonexistent"})
