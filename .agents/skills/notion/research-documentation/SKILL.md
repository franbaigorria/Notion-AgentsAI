...
This skill uses legacy helper labels from the original Claude distribution. In Codex, treat references like `Notion:notion-search`, `Notion:notion-fetch`, and `Notion:notion-create-pages` as placeholders for the real tools exposed by the active `notion` MCP server.

Always inspect the available Notion MCP tools first and then use those real tool names for the workflow below.
