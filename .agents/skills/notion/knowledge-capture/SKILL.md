...
References below to helper names like `Notion:notion-search`, `Notion:notion-create-pages`, and `Notion:notion-update-page` are legacy shorthand from the original Claude-oriented version of this skill.

In Codex, use the active `notion` MCP server instead. Inspect the available Notion MCP tools first, then map those legacy references to the real search, read, create, and update tools exposed in the current session.
