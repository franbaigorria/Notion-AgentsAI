...
This skill has been ported into Codex from a Claude-oriented package. References to helper names such as `Notion:notion-search`, `Notion:notion-fetch`, `Notion:notion-create-pages`, and `Notion:notion-update-page` are legacy shorthand only.

In Codex, inspect the current `notion` MCP server tools and map those conceptual steps to the real search, fetch/read, create, and update operations available in the session.
