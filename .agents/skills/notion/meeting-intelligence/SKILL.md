...
This skill was imported from a Claude-focused package. References to helper names such as `Notion:notion-search`, `Notion:notion-fetch`, and `Notion:notion-create-pages` should be treated as conceptual steps, not literal tool names.

In Codex, inspect the active `notion` MCP server and use the real tool names it exposes for search, fetch/read, and page creation.
